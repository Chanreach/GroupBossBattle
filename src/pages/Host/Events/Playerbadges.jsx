...
import { ArrowLeft, Edit, Trophy, Users, Award, Search, Skull, Target, Crown, Medal, TrendingUp, Star, ChevronLeft, ChevronRight, Clock } from 'lucide-react';
...
const PlayerBadges = () => {
  ...
  const getEarnedBadgeCount = () => {
    return players.reduce((total, player) =>
      total + player.badges.filter(badge => badge.earned).length, 0
    );
  };
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Track individual boss badges and event milestone progress for each player
              </p>
                                            ...
                                            {groupBadges.map((badge, badgeIndex) => {
                                              ...
                                              return (
                                                <Tooltip key={badgeIndex}>
                                                  <TooltipTrigger asChild>
                                                    <div className="flex items-center gap-1">
                                                      <div className="relative">
                                                        <div
                                                          className={`w-8 h-8 rounded-full flex items-center justify-center ring-1 ring-inset transition-all ${badge.earned
                                                  ...
                                                  <TooltipContent>
                                                    <p className='text-center'>{badge.name}<br/><b>{badge.redeemed ? 'Redeemed' : badge.earned ? 'Earned' : 'Locked'}</b></p>
                                                  </TooltipContent>
                                                </Tooltip>
                                              );
                                            })}
                                            ...
                                            {milestoneBadges.map((badge, badgeIndex) => {
                                              ...
                                              return (
                                                <Tooltip key={badgeIndex}>
                                                  ...
                                                  <TooltipContent>
                                                    <p>{badge.name} {badge.redeemed ? '(Redeemed)' : badge.earned ? '(Earned)' : '(Locked)'}</p>
                                                  </TooltipContent>
                                                </Tooltip>
                                              );
                                            })}
