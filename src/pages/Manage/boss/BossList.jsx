...
const BossList = () => {
  ...
  const handleBossClick = (bossId) => {
    navigate(`/manage/bosses/${bossId}/edit`);
  };

  const handleCreateBoss = () => {
    navigate("/manage/bosses/create");
  };

  if (loading) {
    return (
      <div className="container mx-auto px-4 sm:px-6 py-6 max-w-6xl">
        <StatusOverlay message="Loading bosses..." type="loading" />
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="container mx-auto px-4 sm:px-6 py-6 max-w-6xl">
        <StatusOverlay message={error} type="error" onRetry={fetchBosses} />
      </div>
    );
  }
